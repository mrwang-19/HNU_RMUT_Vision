...
$$
0.785 \times \sin (1.884 \times t+\varphi) + 1.305
$$
则需要拟合的式子为，其中$\tau=0.5$：
$$
\int_{t-\tau}^{t}[0.785 \times \sin (1.884 \times t+\varphi) + 1.305] dt\\
=1.305τ+0.41666\dots \left(-\cos \left(φ+1.884t\right)+\cos \left(φ+1.884\left(-τ+t\right)\right)\right)
$$
![image-20210805234313091](./imgs/image-20210805234313091.png)

实际测试时采集1.5s内的角度差进行拟合得到的相位较为准确。由于当$\tau=0.5s$ 时，上式（图中绿色曲线）取值不超过$1.5\pi$，因而添加样本时可以此滤除一部分非预期的数据，确保拟合得到的相位准确可信。
